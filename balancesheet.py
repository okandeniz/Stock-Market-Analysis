...
def bilanco_cekme(hisseler):
    for i in hisseler:
        ...
        df = veri3.loc[:, veri3.columns[::-1]]

        if "2019/3" in df.columns.tolist():
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2019/3"):]
        elif "2020/3" in df.columns.tolist():
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2020/3"):]
        elif "2021/3" in df.columns.tolist():
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2021/3"):]
        elif "2022/3" in df.columns.tolist():
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2022/3"):]
        elif "2023/3" in df.columns.tolist():
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2023/3"):]
        elif "2024/3" in df.columns.tolist():
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2024/3"):]
        else:
            cols = df.columns.tolist()
            new_cols = cols[cols.index("2025/3"):]
